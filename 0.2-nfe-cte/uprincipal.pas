...
uses
  ...
  ACBrCTe, ACBrCTeDACTeRLClass, ACBrDANFCeFortesFr, ACBrNFeDANFeRLClass,
  LCLIntf, StdCtrls, ComCtrls, ExtCtrls, ActnList, Menus, pcnConversao;
...
type
  ...
  TfrmPrincipal = class(TForm)
    ...
    procedure btConvertCTeClick(Sender: TObject);
    procedure btConvertNFeClick(Sender: TObject);
    procedure FormCreate(Sender: TObject);
    procedure Image1Click(Sender: TObject);
    procedure mmConsNFeClick(Sender: TObject);
    procedure mmConsCTeClick(Sender: TObject);
...
var
  ...
  NFePathSchemas_, CTePathSchemas_,
  DANFEPath_, DACTEPath_: string;
  //
...
procedure TfrmPrincipal.btConvertNFeClick(Sender: TObject);
var pathStr,fileStr: string;

begin
  OpenDialog1.Title := 'Escolha um XML de Nota Fiscal Eletrônica';
...
procedure TfrmPrincipal.btConvertCTeClick(Sender: TObject);
var pathStr,fileStr: string;

begin
  OpenDialog1.Title := 'Escolha um XML de Conhecimento de Transporte Eletrônico';
  OpenDialog1.DefaultExt := '*.xml';
  OpenDialog1.Filter := 'Arquivos CTe (*.xml)|*.xml';
  if OpenDialog1.Execute then
  begin
    try
      ACBrCTe1.Conhecimentos.Clear;
      ACBrCTe1.Conhecimentos.LoadFromFile(OpenDialog1.FileName);
      ACBrCTe1.Configuracoes.Arquivos.PathSalvar:= '/';
      ACBrCTe1.Conhecimentos.ImprimirPDF;
      //
      pathStr:= '';
      fileStr:= '';
      //pathStr:= ExtractFileDir(Paramstr(0));
      fileStr:= ACBrCTeDACTeRL1.ArquivoPDF;
      //fileStr:= Copy(fileStr, 2, Length(fileStr));
    finally
      if fileStr <> '' then
      begin
      ...
      begin
        Beep;
        ShowMessage('O arquivo selecionado não é um XML de CT-e válido!')
      end;
    end;
  end;
end;
...
procedure TfrmPrincipal.mmConsNFeClick(Sender: TObject);
...
procedure TfrmPrincipal.mmConsCTeClick(Sender: TObject);
var chaveStr, retornoWS, statusWS: string;
    Arq: TextFile;
    p,q: Integer;

begin
  OpenDialog1.Title := 'Escolha o XML do Conhecimento de Transporte Eletrônico';
  OpenDialog1.DefaultExt := '*.xml';
  OpenDialog1.Filter := 'Arquivos CT-e (*.xml)|*.xml';
  ...
  begin
    ...
    begin
      ...
      //ShowMessage(chaveStr);
      ACBrCTe1.Conhecimentos.Clear;
      ACBrCTe1.WebServices.Consulta.CTeChave:= chaveStr;
      ACBrCTe1.WebServices.Consulta.Executar;

      retornoWS:= ACBrCTe1.WebServices.Consulta.RetornoWS;
      mmXML.Lines.Add('===== Inicio da resposta da SEFAZ =====');
      mmXML.Lines.Add(retornoWS);
      mmXML.Lines.Add('=====  Fim da resposta da SEFAZ  =====');
...
begin
  try
    ...
    Writeln(Arq, 'CTePathSchemas=.\Schemas\CTe\');
    Writeln(Arq, 'DANFEPath=.\nfe\');
    Writeln(Arq, 'DACTEPath=.\cte\');
    CloseFile(Arq);
  except
...
begin
  ...
  CTePathSchemas_:=     ArqIni.ReadString('CAMINHOS', 'CTePathSchemas', '.\Schemas\CTe\');
  DANFEPath_:=          ArqIni.ReadString('CAMINHOS', 'DANFEPath', '.\nfe\');
  DACTEPath_:=          ArqIni.ReadString('CAMINHOS', 'DACTEPath_', '.\cte\');
  ...
  //**************
  //CARREGA INFORMAÇÕES DO CERTIFICADO
  ACBrCTe1.Configuracoes.Geral.SSLLib             := TSSLLib(Cert_SSLLib_);
  ACBrCTe1.Configuracoes.Geral.SSLCryptLib        := TSSLCryptLib(Cert_CryptLib_);
  ACBrCTe1.Configuracoes.Geral.SSLHttpLib         := TSSLHttpLib(Cert_HttpLib_);
  ACBrCTe1.Configuracoes.Geral.SSLXmlSignLib      := TSSLXmlSignLib(Cert_XmlSignLib_);
  //  ACBrCTe1.SSL.SSLType                            := LT_TLSv1_2;
  //ACBrCTe1.Configuracoes.Certificados.URLPFX      := uPrincipal.Cert_URLPFX_;
  ACBrCTe1.Configuracoes.Certificados.ArquivoPFX  := Cert_Caminho_;
  ACBrCTe1.Configuracoes.Certificados.Senha       := Cert_Senha_;
  //ACBrCTe1.Configuracoes.Certificados.NumeroSerie := Cert_NumSerie_;

  //CONFIGURA CAMINHOS
  ACBrCTe1.Configuracoes.Arquivos.PathSchemas     := CTePathSchemas_;
  ACBrCTeDACTeRL1.PathPDF                         := DACTEPath_;
  if not DirectoryExists(DACTEPath_) then
    CreateDir(DACTEPath_);
  ...
  //CONFIGURA SSL
  ACBrCTe1.SSL.SSLType := TSSLType(WS_SSLType_);
end;
